...
contract Invariant is InvariantTest, Test {
    ...
    GoatTypes.InitParams initParams = GoatTypes.InitParams({
    ...
    function setUp() public {
        ...
        selectors[1] = handler.swapTokenToWeth.selector;
    ...
    function invariantReserves() public {
        ...
        assertEq(handler.expectedDeltaTokenReserve(), handler.expectedDeltaTokenReserve());
    }

    function invariantFees() public {
        assertApproxEqAbs(handler.expectedTotalFees(), handler.actualTotalFees(), 1);
        uint256 totalFees = pair.getPendingLiquidityFees() + pair.getPendingProtocolFees();
        assertApproxEqAbs(handler.expectedTotalFees(), totalFees, 1);
    }

    function invariantBootstrapWethShouldBeGreaterThanRealWethReserveInPresale() public {
    ...
    function invariantBootstrapWethShouldBeLessThanOrEqualRealWethReserveInAmm() public {
        if (pair.vestingUntil() != type(uint32).max) {
            uint256 wethBalance = weth.balanceOf(address(pair));
            uint256 protocolFees = pair.getPendingProtocolFees();
            uint256 actualRealWethReserve = wethBalance - protocolFees;
    ...
    function invariantTokenBalanceInPairShouldNeverBeGreaterThanBootstrapTokenAmountInPresale() public {
        if (pair.vestingUntil() == type(uint32).max) {
            uint256 tokenBalance = token.balanceOf(address(pair));

            (,, uint112 virtualEth, uint112 initialTokenMatch, uint112 bootstrapEth,) = pair.getStateInfoForPresale();
            GoatTypes.InitialLPInfo memory initialLpInfo = pair.getInitialLPInfo();
            uint256 actualBootstrapTokenAmount = GoatLibrary.getActualBootstrapTokenAmount(
                virtualEth, bootstrapEth, initialLpInfo.initialWethAdded, initialTokenMatch
            );
